...
function buildSystemPrompt({ startingPrice, floorPrice, absoluteFloor, turnCount, maxTurns, displayName }) {
  ...
  return [
    ...
    displayName
      ...
      : null,
    // isLastTurnの指示は、プロンプトの中で埋もれて無視されないよう、意図的に一番最後
    // (tool呼び出し指示の直前)に置いている。実際に「これでどうだ?」のような疑問形で
    // 終わってしまう事例があったため(2026-07-16)、禁止パターンを増やし、断定文の
    // テンプレートを複数提示して「的屋の伝法な口調」でも断定できる形を具体的に示した。
    `これは最大${maxTurns}回の会話のうち${turnCount + 1}回目のやり取りです。会話が進むほど残りのやり取りが減るので、終盤は価格を固めていってください。`,
    isLastTurn
      ? '最重要(このターンだけの特別ルール、必ず守ること): これが最後のやり取りです。次はありません。replyは疑問形で終わらせては絶対にいけません。「〜しようか」「〜でどう?」「どうだ?」「〜だろ?」「〜かい?」のように、まだ迷っている・提案しているだけ・相手の返事を待っているように聞こえる言い回しは、勢いのいい伝法な口調であっても一切禁止です(文末に「?」を付けるのも禁止)。必ず「よし、決まりだ!◯◯ICHIGOだ!」「しゃあねえ、◯◯ICHIGOで手を打とう、それ以上は無しだ!」のような、疑問形を含まない断定的な一文で最終価格をはっきり宣言し、交渉をきっぱり締めくくってください(◯◯には実際のpriceの数値を入れる)。'
