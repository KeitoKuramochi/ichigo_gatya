...
### 【実機で発覚・修正】RPC_URLの無料公開エンドポイントがVercelからブロックされる問題(2026-07-19)

ユーザーが実際にVercelへデプロイして試したところ、「この価格で決める」を押すと
`could not decode result data (value="0x", ..., code=BAD_DATA)`というエラーになった。
`curl`で自分の環境から同じRPC(`mainnet.optimism.io`)+同じトークンアドレスにdecimals()を
問い合わせたところ正常に応答したため、ローカルでは再現しない=Vercel側の環境固有の問題と判断。

- 別の無料公開RPC(`drpc.org`)に切り替えても症状が変わらなかったため、応答に
  `debugRpcHost`/`debugTokenAddr`(実際に使われている値)を一時的に含めるデバッグ版を
  デプロイして原因を特定: **RPC自体は問題なくAlchemy(ユーザーが新規に無料アカウントで
  発行)に切り替わっていたが、`TOKEN_ADDR`環境変数の値が誤って`GAME_WALLET`と同じ
  ウォレットアドレスになっていた**(コントラクトではなく通常のウォレットアドレスに
  decimals()を問い合わせていたため、当然0x(空)が返っていた)。ユーザーが環境変数を
  手入力する過程で2つの値を取り違えたことが原因で、RPCプロバイダの問題ではなかった。
- `TOKEN_ADDR`を正しい値(`0x836700463Dce76D9Cc3CDf6F6EDF946312c01869`)に修正して解決。
  実際の送金(2000 ICHIGO)→検証→景品「明日への扉(激エモ)」の表示・ダウンロードまで
  E2Eで成功を確認(ユーザーからのスクリーンショットで確認)。
- 副産物として、以下の恒久的な改善を行った: (1) `.env.example`にAlchemy等APIキー方式の
  RPCを強く推奨する注記を追加(無料公開RPCはクラウドIPからのアクセスをブロックすることが
  ある教訓を反映)、(2) `applyFinalPrice`(オンチェーンRPC呼び出しを含む)で例外が起きた際、
  従来は握りつぶされてVercelの汎用500(FUNCTION_INVOCATION_FAILED、プレーンテキスト)が
  返り「フロント側がres.json()でパース失敗して原因不明のエラーになる」という不具合が
  `online-negotiate-finalize.js`・`online-negotiate-message.js`にあったため、
  try/catchで実際のエラー内容をJSONとして返すよう修正(デバッグ用の`debugRpcHost`等の
  一時フィールドは原因特定後に削除済み)。

## 「集めた景品」機能(モザイク解除・再ダウンロード・当選アナウンス)を追加(2026-07-19)

実際にE2Eで動作確認が取れた直後、「自分のウォレットで当てた景品はトップページで
モザイクが解けて、いつでも押せば再ダウンロードできるようにしてほしい。誰かが当てた時の
アナウンスもトップページに出してほしい」という要望を受けて実装した。

- **バックエンド**: `api/_lib/store.js`に`recordWonPrize`/`listWonPrizes`を追加。
  ウォレットごとの当選記録をRedisのList(`online:won:<wallet>`)にTTL無しで永続保存する
  (セッション自体は20分で消えるが、こちらは「いつでも見返せる」という要件のため無期限)。
  `online-verify-and-claim.js`の決済確認成功時に、`pushReveal`(既存の投影用キュー)に
  加えて`recordWonPrize`も呼ぶよう変更。新規エンドポイント`GET /online-my-prizes?wallet=`
  (認証無し、walletは元々公開情報なので問題ない)を追加し、`vercel.json`にrewriteを追加。
- **フロントエンド(`index.html`)**: `loadPrizeGallery()`が生成する各カードに
  `data-prize-id`を持たせ、`prizeCardEls`に控えておく。ウォレット接続成功時に
  `loadMyPrizes(account)`を呼び、返ってきた当選済みのprize.idに対応するカードだけ
  `revealOwnedCard()`でモザイク(`.prizeMosaic`のCSS blur)・シェード・「?」を外して
  実画像+獲得済みバッジ+タップで`downloadImage()`(`<a download>`を動的生成してクリック)
  という形に差し替える。景品ギャラリー(`#introSection`)はウォレット接続直後に
  操作性のため畳んでいた(前回実装)ため、常設の**「🍓 集めた景品を見る」ボタン**
  (`#myCollectionBtn`、接続後に表示)を新設し、いつでも`#introSection`を再表示して
  見返せるようにした。
- **当選アナウンス**: 新規エンドポイントは追加せず、投影ページ用に元々あった
  `/online-negotiate-current`の`reveals`配列をトップページ自身もポーリング(4秒間隔)する
  形にした。ページを開いた時刻より後の`revealedAt`だけを新規として扱い、
  画面上部固定の`#announceToast`(誰の当選でも公開表示、認証不要)に
  「🎉 ◯◯さんが「△△」を当てました」のようにトースト表示→4.5秒で自動的に消える。
  特別当選(手描き)の場合は見た目を金色に変える。
- 動作確認: Playwrightで偽のEIP-1193プロバイダ(`window.ethereum`)を注入してウォレット
  接続を模擬し、`/online-my-prizes`・`/online-negotiate-current`をモックした上で
  実際のJSロジック(`loadPrizeGallery`→`revealOwnedCard`→カードのDOM状態)を検証: 当選済み
  2種類(通常+スペシャル)のカードだけが`owned`(+`special`)クラスを持つことを確認。
  トースト表示→4.5秒後の自動非表示も、DOM状態を1秒おきに直接ポーリングするテストで
  正確なタイミングを確認済み。確認用スクリプトは`.design-qa/collection-check.mjs`・
  `.design-qa/toast-debug.mjs`(いずれもgitignore対象)。実際のRedisに対する
  `recordWonPrize`/`listWonPrizes`の疎通はデプロイ後にユーザー側で要確認。

### 公開アナウンス機能の撤去・支払いフローの導線修正(2026-07-19、続き)

ユーザーの実機テスト(本番Vercel環境、実ウォレット)から3件のフィードバックを受けて修正した。

- **「他の人の報告機能は消してください」**: 追加したばかりの公開アナウンス(誰かが当てると
  画面上部にトースト表示)機能を全面撤去。`#announceToast`のHTML/CSS、
  `showAnnouncement`/`pollAnnouncements`とその`setInterval`をindex.htmlから削除
  (バックエンドの`/online-negotiate-current`自体は元々投影ページ用に存在したものなので
  無変更で残した。フロント側の利用をやめただけ)。「自分が当てた景品をトップページで
  見返せる」機能(集めた景品を見るボタン)は別機能として要望通り維持。
- **「支払おうとすると支払い画面が出てこない、やめるを押すと遅れて出てくる」**:
  `finalizeBtn`(「この価格で決める」)クリック時、`/online-negotiate-finalize`の
  応答を待つ間(オンチェーンRPC呼び出し=`decimals()`取得を含むため通信状況次第で
  数秒かかりうる)に何の状態表示も出しておらず、ボタンのdisabled状態(見た目の変化が
  乏しい)だけが手がかりだったため「固まった」ように見えていたと判断。
  `showStatus("価格を確定しています…", "")`をfetch呼び出し前に追加し、体感の
  レスポンス性を改善した。Playwrightで`/online-negotiate-finalize`の応答を意図的に
  1.8秒遅延させるモックを使い、遅延中に文言が表示されること・遅延後に支払い画面へ
  正しく遷移することを確認済み。
- **「獲得した一覧を見た後、引くボタンが無く、また一覧を見るボタンしかない」**:
  ウォレット接続直後に導入セクション(景品ギャラリー)を畳む改善(前回実装)の副作用で、
  「集めた景品を見る」で導入セクションに戻った後、次の交渉を始める手段が無い
  行き止まりになっていた。ウォレット接続時とボタン起点の両方から呼べる共通関数
  `enterNegotiationFlow()`に処理を切り出し、景品ギャラリー内に新規ボタン
  **「もう一度ガチャを引く」**(`#drawAgainBtn`)を追加。押すと保存済みセッションが
  あれば復帰、無ければ呼び名入力からやり直せる。あわせて`myCollectionBtn`の
  トグルクローズ(押すたびに表示/非表示を切り替える、行き止まりの一因だった)仕様は
  やめ、常に導入セクションを表示する単純な動作に変更した。
- 動作確認: Playwrightで偽のEIP-1193プロバイダにより接続→名前入力→チャット→
  「この価格で決める」(遅延モック)→支払い画面遷移までを実際のクリック操作で確認
  (`.design-qa/full-flow-check.mjs`)。「集めた景品を見る」→「もう一度ガチャを引く」の
  往復も、各セクションの表示/非表示状態をアサーションで確認済み
  (`.design-qa/draw-again-check.mjs`)。`tx.wait()`(送金確定待ち)以降はモックの限界で
  自動確認していないが、ユーザー自身が本番環境で実送金・受け取りまで成功済みのため
  実害は無いと判断。

## story/(別リポジトリ`ichigo_gatya_web`)をオンライン版向けに更新(2026-07-20)

「挑戦ページ(`https://ichigo-gatya-web.vercel.app/`)もオンライン向けにしてほしい」との
要望を受けた。このページは`ichigo`本体とは別のGitHubリポジトリ(`ichigo_gatya_web`、
ローカルでは`story/`ディレクトリにクローン済み)で、Vercelで独立デプロイされている
「つくった話・遊び方」の紹介ページ。展示当日の現地実機版を前提にした文言のままだった。

- 「挑戦する」CTAリンク(ヘッダーnav・本文・フッターの計3箇所)を、すべて現地版
  (`https://ichigo-gatya.onrender.com/negotiate/`)から**オンライン版
  (`https://ichigo-gatya.vercel.app/`)**に差し替え。
- トップのリード文に「展示は終わりましたが、オンライン版の延長戦でいつでも挑戦できます」
  を追加(「待ち時間にゆっくりどうぞ」という現地の行列前提の文言を置き換え)。
- 「しくみ」セクション: 現地版(実機ロック解除)の説明はそのまま残しつつ、
  「オンライン版(延長戦)では、実機の代わりに景品の記念画像がその場で表示されます」を
  追記(図解(SVG)自体は現地版の説明として変更していない)。
- 「あそびかた」を5ステップ(QR読み取り→接続→交渉→支払い→レバーを回してカプセルを
  受け取る)から**4ステップ**(サイトを開いてウォレット接続→交渉→支払い→景品の記念画像を
  受け取る)に再構成。物理QR・レバー・カプセルへの言及を削除し、online-vercel側の
  あそびかた説明と同じ内容に揃えた。
- 動作確認: Playwright(`.design-qa/story-check.mjs`)で実際にページを開き、3箇所の
  CTAリンクがすべて新URLになっていること、あそびかたセクションが4ステップで正しく
  表示されることをスクリーンショットで確認済み。
- **未実施**: `story/`はichigo本体と別リポジトリ(`ichigo_gatya_web`)のため、
  コミット・pushはユーザーの確認を得てから別途行う。

## online-vercel: 接続直後を「ホーム画面」化、履歴表示、再接続不要の導線に再設計(2026-07-20)

ユーザーからのフィードバックを受けて、ウォレット接続直後の体験を大きく作り直した。

- **「景品を見るボタン」を廃止し、接続したら最初から出しておく方式に変更**:
  旧`introSection`(遊び方説明+ギャラリー)を`pitchSection`(接続前のみ表示する
  レード文+あそびかた4ステップ)と、新設の`dashboardSection`(接続後に表示する
  ホーム画面: 🎰ガチャを回すボタン・直近の記録・景品ギャラリー)に分割。
  接続直後は自動でチャットへ進まず、この「ホーム画面」を表示するようにした
  (保存済みの交渉中セッションがある場合のみ、そこへ自動復帰する動作は維持)。
  `myCollectionBtn`(景品を見る)・`drawAgainBtn`(もう一度ガチャを引く)は廃止し、
  常時表示の`homeBtn`(🏠ホームに戻る、交渉中でもいつでも押せる)と
  `spinBtn`(🎰ガチャを回す、ホーム画面内)に統合。
- **直近のガチャ履歴を追加**: `online-verify-and-claim.js`の`recordWonPrize`呼び出しに
  支払い金額(`price`)を追加(元々レスポンス用に計算済みの`ethers.formatUnits`の値を
  再利用)。フロント側は既存の`/online-my-prizes`(景品ギャラリーのモザイク解除と同じ
  データソース)を使って、日時・支払い金額・景品名を新設の`#historyList`に最大5件表示する
  (`renderHistory()`、`loadMyPrizes()`と同じ1回のfetchを共用)。
- **直筆(手描きスペシャル)当選をより豪華な見た目に変更**: 当選演出画面(prizeReveal)で
  既に使っていた`specialGlow`(発光パルス)・`specialShimmer`(金のグラデーション文字が
  流れる)のCSSアニメーションをそのまま再利用し、ギャラリーの獲得済みカードにも適用。
  スパークル(✨)も2つ追加。通常当選(赤バッジ)と直筆当選(金の輝き)が視覚的にはっきり
  差別化されるようにした。
- **「もう一度ガチャを引く」で再接続を要求しない設計に変更**: レシート画面の
  「トップへもどる」ボタンが`location.reload()`を呼んでいたため、押すたびにMetaMaskの
  再接続(承認)が必要になっていた。account/signer/tokenはページをリロードしなければ
  メモリ上に残っているため、reloadをやめて「ホーム画面を表示し直すだけ」の処理に変更。
  ホーム画面の🎰ガチャを回すボタン、🏠ホームに戻るボタンも同様に、一度接続すれば
  ページ内で完結し、二度目以降の`eth_requestAccounts`呼び出しは発生しない。
- 動作確認: Playwrightで偽のEIP-1193プロバイダを使い、(1)接続後にチャットへ自動遷移
  せずホーム画面(ガチャを回す・履歴3件・ギャラリー)が表示されること、(2)「ガチャを回す」
  →「ホームに戻る」の往復で`eth_requestAccounts`が1回しか呼ばれないこと、
  (3)レシート画面から「トップへもどる」を押してもページ遷移(load イベント)も
  再接続も発生せず、更新された履歴・残高とともにホーム画面に戻ることを、
  DOM状態のアサーションとスクリーンショットの両方で確認済み
  (`.design-qa/dashboard-check.mjs`・`.design-qa/buyagain-check2.mjs`)。
  `tx.wait()`(送金確定待ち)を含む決済シミュレーションはモックの限界で自動テストできて
  いないが、支払いより前後の導線(ホーム↔交渉、レシート→ホーム)はこの制約の外なので
  影響を受けない。

### キャラ画像の状態リセット漏れを修正、交渉の残りターン表示を全面改善(2026-07-20)

- **バグ修正**: 景品お披露目時の「お礼(万歳)」ポーズ(`setCharacterImage("thanks")`)から、
  ホームに戻っても最初の「いらっしゃい」ポーズに戻らず出しっぱなしになっていた。
  `showHome()`に`setCharacterImage("greet")`を追加して解決。
- **「ターン数の表示が小さくてわかりにくく、何の予告もなく交渉が終わったように見える」との
  指摘を受けて全面改善**: 従来は価格表示の横に小さく「(1/4ターン)」と出すだけだった。
  - チャット欄の直上に、常時見える大きさの「のこり◯回」表示(ドット4つで進捗も視覚化)を新設。
  - チャット開始時に「会話は最大4回まで。回数を使い切ると、その時点の価格で自動的に確定
    します」という説明文を常時表示するようにした(一度きりの案内ではなく、ずっと見える形)。
  - **残り1回になったら**、のこり回数バーが赤くパルスするように変化し、
    「⚠️ 次のひとことが最後です!悔いのないように」という目立つ警告文が出るようにした。
    これにより、最終ターンで自動的に価格が確定→支払い画面に切り替わる仕様自体は
    変えていないが、参加者が「まだ話せると思ったら急に終わった」と感じないようにした。
- 動作確認: Playwrightで実際に4ターン分のチャットを送信し、各ターンでドット・残り回数
  テキスト・最終ターン警告の表示/非表示が正しく切り替わることをスクリーンショットで確認済み
  (`.design-qa/turn-meter-check.mjs`)。

### 呼び名の文字種制限を撤廃、同時接続上限を現地版の名残(2人)から実質無制限へ(2026-07-20)

「もうオンライン用だから、同時接続人数は何人でもいいし、名前も漢字とか何でもいいのでは」
という指摘を受けて2点変更した。

- **呼び名の文字種制限を撤廃**: フロント側の`DISPLAY_NAME_PATTERN`
  (`/^[0-9A-Za-z぀-ヿ ]*$/`、ひらがな・カタカナ・半角英数字のみ)が、現地版のESP32 OLED
  フォント制限(`u8g2_font_unifont_t_japanese1`が漢字非対応)の名残のまま持ち込まれていた。
  バックエンド側(`api/_lib/util.js`の`sanitizeDisplayName`)は元々このデプロイ用に
  制御文字だけ弾く形で実装済みだったため、フロント側も同じ基準(制御文字のみ拒否)に
  揃え、漢字・絵文字を含め自由に入力できるようにした。入力欄の説明文・エラー文言も
  「ひらがな・カタカナ・半角英数字のみ」から「お好きな文字で入力できます」に変更。
- **同時接続上限のデフォルトを2→100に変更**: `ONLINE_MAX_CONCURRENT`は現地版
  (実機1台=同時1交渉が前提の設計を踏襲し、当初「行列ができないように」2としていた)の
  名残の値だった。オンライン専用になった今は物理的な制約が無いため、実質無制限とみなせる
  値(100)をデフォルトにした(0や超巨大値にしない理由: AI API呼び出しの異常系を完全に
  無視しないための緩い安全弁として、上限の仕組み自体は残す判断)。
  **申し送り**: Vercelの環境変数に`ONLINE_MAX_CONCURRENT=2`を明示的に設定済みの場合、
  環境変数がコードのデフォルト値より優先されるため、値を書き換える(または削除する)
  必要がある。
- 動作確認: Playwrightで漢字の表示名("倉持啓人")を送信し、エラー無く交渉が開始される
  ことを確認済み。

### 重複当選の回数表示を追加(2026-07-20)

「同じ景品が重複して出た場合、その回数もトップページに表示できたら面白い(確率は変えない)」
という要望を受けて実装。`/online-my-prizes`のレスポンスは元々ウォレットの当選記録を
全件(重複含む)返しており、`loadMyPrizes()`側で同じidは1枚のカードにまとめて表示するため
重複が捨てられていただけだったので、抽選ロジック(`prize-pool.js`)には一切手を入れず、
表示側の集計だけで実現できた。

- `loadMyPrizes()`でid別の件数を集計し、`revealOwnedCard(prize, count)`に渡すように変更。
- `count > 1`の場合のみ、獲得済みバッジ(左上)とは別に右上へ`×3`のような回数バッジ
  (`.dupCount`、金文字)を表示する。1回だけの場合は何も出さない(情報量を増やしすぎない)。
- 動作確認: Playwrightで同じ景品を3回・2回当てたダミーデータを与え、該当カードにのみ
  `×3`/`×2`バッジが正しく表示されることをスクリーンショットで確認済み。

併せて、ユーザーからの「Redis(Upstash)の無料枠に上限はあるか」という質問にWebFetchで
実際の料金ページを確認して回答: 最大データサイズ256MB・月間50万コマンド・月間帯域幅10GB・
DB1個まで。本アプリはポーリングを行わない設計(以前あった当選アナウンスの定期ポーリングは
既に撤去済み)のため、コマンド消費はユーザー操作に比例するだけで、講義規模の利用であれば
無料枠で十分という判断を伝えた。
