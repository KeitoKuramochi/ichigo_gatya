...
- `bridge/server.js`の`UNLOCK_ESP32_TIMEOUT_MS`を15秒→30秒に、`negotiate/index.html`の`UNLOCK_POLL_MAX_TRIES`を20→30(約45秒)に、それぞれ余裕を持たせて修正した。
...
- 未実施: 実機での再現テスト(タイムアウトを30秒に広げたことで実際に間に合うようになるかは、次回の実機テストで確認すること)。
