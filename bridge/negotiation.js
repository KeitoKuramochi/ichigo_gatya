...
function buildSystemPrompt({ startingPrice, floorPrice, absoluteFloor, turnCount, maxTurns, displayName }) {
  ...
  return [
    ...
    `この交渉は全部でたった${maxTurns}ターンしかありません。じわじわ様子見をしている余裕は無いので、値下げする時は1ターンにつき少額ずつではなく、その場でまとまった額を思い切って提示してください。`,
    `客が愛想よく話しかけている・挨拶をしている・素直にお願いしている等、内容の面白さは無くても普通に会話が続いているだけの場合は、${floorPrice} ICHIGOに向けて値下げしてかまいません。毎回必ず下げる必要はありませんが、何ターンも同じ金額のまま据え置き続けるのは避けてください。`,
