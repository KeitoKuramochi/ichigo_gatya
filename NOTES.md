...
- `firmware/gachapon/gachapon.ino`: 本番用。WiFi接続+`POST /unlock`(要`X-Secret`ヘッダー)でサーボを解除動作させる
...
## モーター調整用テストページ(一旦revert、2026-07-03)

サーボの解除角度・保持時間を実機で試行錯誤するための専用ページを一度実装したが、有効化にはESP32への書き込み直しが必要なため、直近では書き込みをしない方針となり実装を一旦revertした(`test/motor-test.html`削除、`bridge/server.js`/`firmware/gachapon/gachapon.ino`のテスト動作関連コードを除去)。次にESP32を書き込むタイミングでまとめて再導入する。

- 実装済みのコードはgit履歴のコミット`679d3f1`(のちfirmware/bridgeの該当差分のみrevertするコミット)に残っているので、再度必要になったら`git show 679d3f1 -- bridge/server.js firmware/gachapon/gachapon.ino test/motor-test.html`で内容を確認し、該当差分を再適用すればよい
- 機能の概要: `test/motor-test.html`(bridgeが`/test/`以下で配信)で角度(0〜180度)・保持時間(秒)を指定して「動かす」を押すと、bridgeの`POST /test-move`(要`X-Secret`)で予約され、ESP32が既存の`/poll-unlock`ポーリングに相乗りする形で拾って(`testMove`/`testRequestId`/`testAngle`/`testHoldMs`フィールド)サーボを動かし、`POST /test-move-result`で結果報告、ページは`GET /test-move-status`でポーリング確認する
- 相乗り方式にした理由: ESP32はHTTPS通信を数秒おきに繰り返すとヒープが減っていく既知の問題を抱えており、テスト動作のためだけに別のポーリング先を増やすとHTTPSリクエスト回数が倍になり本番の解除フローの安定性を損なうため(再導入時もこの設計を踏襲すること)
...
- 2026-07-03: モーター(サーボ)の解除角度・保持時間をブラウザから調整できるテストページ`test/motor-test.html`を追加。bridgeに`/test-move`・`/test-move-status`・`/test-move-result`を新設し、ESP32への配信は既存の`/poll-unlock`に相乗りさせる形にした(HTTPSポーリング回数を増やしてヒープ枯渇リスクを上げないため)。ローカルで別ポート(3099)にbridgeを起動し、`curl`で予約→ポーリング→結果報告→ステータス確認の一連の流れを確認済み(既存の本番用bridgeプロセスは起動したまま、干渉させていない)。あわせて、以前から未コミットだった決済ページの解除確認待ち機能(`payment/index.html`の`/unlock-status`ポーリング、bridgeの`unlockRequests`状態管理)も一緒にcommit・push(コミット`679d3f1`)し、Render.comの本番bridgeに反映した。
- 2026-07-03: モーターテスト機能は、有効化に実機ESP32への書き込み直しが必要なこと・直近では書き込みをしない方針であることから、`test/motor-test.html`削除・`bridge/server.js`と`firmware/gachapon/gachapon.ino`のテスト動作関連コードのrevertを実施(決済ページの解除確認待ち機能はそのまま残した)。再導入手順はNOTES.mdの「モーター調整用テストページ」の項を参照。
